...
public:
	...
	Papyrus(std::string data) {
		...
		if (DataDealer::checkFileExist(std::string(DATA_PATH) + std::string(PAPYRUS_DATA_NAME))) {
			...
			switch (type) {
			case 1:
				count = 100000;//Papyrus离开1605地图，开始1606的时间
				if (mode != PAPYRUS_LEAVING_MODE_2)
					exist = false;
				break;
			case 2:
				count = 100000;//Papyrus离开1606地图，开始1607的时间
	...
	void run(Player &player, GameMessageBox &box) {
		if (exist) {
			switch (count) {
			case 40:
				box.add("你是Sans说的那个新来的坏家伙吗？你怎么跑到这里来了？",90);
				break;
			case 140:
				box.add("看起来你是从Sans那偷偷的溜过来的嘛~", 90);
				mode = PAPYRUS_TALK_MODE_3;
				break;
			case 240:
				box.add("新来的家伙，看起来你也不是很像坏人嘛，你是做过很多恶作剧吗？", 90);
				mode = PAPYRUS_TALK_MODE_1;
				break;
			case 340:
				box.add("偷偷告诉你一个秘密，其实我也经常把Sans的巧克力棒藏起来...", 90);
				break;
			case 440:
				box.add("然后吃掉啦~", 50);
				mode = PAPYRUS_TALK_MODE_3;
				break;
			case 500:
				box.add("你可不要告诉Sans哦...", 90);
				mode = PAPYRUS_TALK_MODE_1;
				break;
			case 600:
				box.add("嗨小家伙，我想我有一点喜欢你了。", 90);
				break;
			case 700:
				box.add("但是要是Undyne知道了肯定会不高兴的吧...", 90);
				break;
			case 800:
				box.add("这样吧，跟我来，我带你去找Asriel。", 90);
				break;
			case 850:
